        ...
        for episode in range(nbr_episode):
            ...
            if episode % 1 == 0:
                if self.environment_name == 'community':
                    info_file = open(str(self.termination_step) + "_community_info_.txt", 'a')
                    info_file.writelines(str(self.env.community_info()))
                    info_file.write('\n')
                    info_file.close()
                if self.environment_name == 'small_world':
                    info_file = open(str(self.termination_step) + "_sw_info_.txt", 'a')
                    info_file.writelines(self.env.small_world_info())
                    info_file.write('\n')
                    info_file.close()
