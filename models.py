...
class S2V_QN_1(torch.nn.Module):
    ...
    def forward(self, preferences, adj): # input: preference weight vector; adjacent matrix of a 2-level ego network (observation)

        minibatch_size = preferences.shape[0]
        nbr_node = preferences.shape[1]


        for t in range(self.T):
            if t == 0:
                #mu = self.mu_1(preferences).clamp(0)
                mu = torch.matmul(preferences, self.mu_1).clamp(0)
                #mu.transpose_(1,2)
                #mu_2 = self.mu_2(torch.matmul(adj, mu_init))
                #mu = torch.add(mu_1, mu_2).clamp(0)

            else:
                #mu_1 = self.mu_1(preferences).clamp(0)
                mu_1 = torch.matmul(preferences, self.mu_1).clamp(0)
                #mu_1.transpose_(1,2)
                # before pooling:
                for i in range(self.len_pre_pooling):
        ...
        q_1 = self.q_1(torch.matmul(preferences.transpose(1,2),mu)).expand(minibatch_size,nbr_node,self.embed_dim)
        q_2 = self.q_2(mu)
